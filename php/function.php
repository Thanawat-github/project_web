...
class lms
{
	...
	// public function paginationimg($table,$rows="*",$where = null,$page_rows=10){
	...
	public function delFileExit($dir_path)
	{
		$data = array();
		if ($dir_path == 'img_cap5min') {
			$sql = "SELECT path_cap FROM checkcap";
			$result = $this->dbConnect->query($sql);
			while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				$data[] = "upload/" . $dir_path ."/".$row['path_cap'];
			}
		} elseif ($dir_path == 'img_student') {
			$sql = "SELECT std_pic FROM student";
			$result = $this->dbConnect->query($sql);
			while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				$data[] = "upload/" . $dir_path ."/".$row['std_pic'];
			}
		} elseif ($dir_path == 'img_teacher') {
			$sql = "SELECT profile FROM teacher";
			$result = $this->dbConnect->query($sql);
			while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				$data[] = "upload/" . $dir_path ."/".$row['profile'];
			}
		} elseif ($dir_path == 'img_subject') {
			$sql = "SELECT image FROM subject";
			$result = $this->dbConnect->query($sql);
			while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				$data[] = "upload/" . $dir_path ."/".$row['image'];
			}
		}

		foreach (glob("upload/" . $dir_path . "/*") as $filename) {
